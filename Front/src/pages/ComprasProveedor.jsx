...
export function Gestion_de_Proveedores() {
  ...
  const handleNewOrder = (supplier) => {
    alert(`Iniciar pedido con: ${supplier.name}`);
    setShowActionMenu(null);
  };

  // Handle order details and status change
  const handleOrderDetails = (supplier, order) => {
    setCurrentSupplier(supplier);
  ...
  return (
    <UI5ThemeProvider>
      ...
      <Container>
                ...
                <tbody>
                  {currentSuppliers.map(supplier => (
                    <tr key={supplier.id}>
                      ...
                      <td>
                        <Button primary onClick={() => handleNewOrder(supplier)}>
                          Pedido
                        </Button>
                        <div 
                          ref={showActionMenu === supplier.id ? menuRef : null}
                          style={{ display: 'inline-block', marginLeft: '0.5rem', position: 'relative' }}
                        >
                          <MenuButton onClick={() => setShowActionMenu(showActionMenu === supplier.id ? null : supplier.id)}>
                            <MoreIcon />
                          </MenuButton>
                          <Menu open={showActionMenu === supplier.id}>
                            <MenuItem onClick={() => handleViewDetails(supplier)}>
          ...
          <DialogBackdrop>
            <Dialog size="lg">
                      ...
                      <tbody>
                        {currentSupplier.orders.map(order => (
                          <tr key={order.id}>
                            ...
                            <td>
                              <Button onClick={() => handleOrderDetails(currentSupplier, order)}>
                                Gestionar
                              </Button>
                            </td>
                          </tr>
              ...
              <DialogFooter>
                ...
                <Button primary onClick={() => handleNewOrder(currentSupplier)}>
                  Iniciar Pedido
                </Button>
              </DialogFooter>
            </Dialog>
        ...
        {/* Order Status Dialog */}
        {showOrderDialog && currentSupplier && currentOrder && (
          <DialogBackdrop>
            <Dialog size="md">
              <DialogHeader>
                <DialogTitle>Gestionar Pedido #{currentOrder.id}</DialogTitle>
                <Button onClick={() => setShowOrderDialog(false)}>✕</Button>
              </DialogHeader>
              <DialogContent>
                <div>
                  <FormGroup>
                    <FormLabel>Proveedor</FormLabel>
                    <div style={{ fontWeight: 600, fontSize: '1.125rem' }}>{currentSupplier.name}</div>
                  </FormGroup>
                  
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.5rem' }}>
                    <FormGroup>
                      <FormLabel>Fecha del Pedido</FormLabel>
                      <div style={{ fontSize: '1rem' }}>{new Date(currentOrder.date).toLocaleDateString()}</div>
                    </FormGroup>
                    
                    <FormGroup>
                      <FormLabel>Productos</FormLabel>
                      <div style={{ fontSize: '1rem' }}>{currentOrder.items} unidades</div>
                    </FormGroup>
                    
                    <FormGroup>
                      <FormLabel>Monto Total</FormLabel>
                      <div style={{ fontSize: '1rem', fontWeight: 600 }}>${currentOrder.amount.toLocaleString()}</div>
                    </FormGroup>
                  </div>
                  ...
                  <FormGroup>
                    ...
                    <StatusIndicator 
                      status={
                        currentOrder.status === 'entregado' ? 'active' :
                        currentOrder.status === 'pendiente' ? 'pending' :
                        currentOrder.status === 'cancelado' ? 'inactive' : 'pending'
                      }
                  ...
                  <FormGroup>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginTop: '0.75rem' }}>
                      <Button 
                        onClick={() => handleOrderStatusChange('pendiente')}
                        primary={currentOrder.status !== 'pendiente'}
                        style={{ opacity: currentOrder.status === 'pendiente' ? 0.5 : 1 }}
                        disabled={currentOrder.status === 'pendiente'}
                      >
                        Pendiente
                      </Button>
                      <Button 
                        onClick={() => handleOrderStatusChange('en_proceso')}
                        primary={currentOrder.status !== 'en_proceso'} 
                        style={{ opacity: currentOrder.status === 'en_proceso' ? 0.5 : 1 }}
                        disabled={currentOrder.status === 'en_proceso'}
                      >
                        En Proceso
                      </Button>
                      <Button 
                        onClick={() => handleOrderStatusChange('entregado')}
                        primary={currentOrder.status !== 'entregado'}
                        style={{ opacity: currentOrder.status === 'entregado' ? 0.5 : 1 }}
                        disabled={currentOrder.status === 'entregado'}
                      >
                        Entregado
                      </Button>
                      <Button 
                        onClick={() => handleOrderStatusChange('cancelado')}
                        danger
                        style={{ opacity: currentOrder.status === 'cancelado' ? 0.5 : 1 }}
              ...
              <DialogFooter>
                <Button onClick={() => setShowOrderDialog(false)}>Cerrar</Button>
              </DialogFooter>
            </Dialog>
          </DialogBackdrop>
        )}
